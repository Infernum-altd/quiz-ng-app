...
export class GameService {
  ...
  CREATE_GAME = `${this.BASE_URL}\\play\\addSession`;
  webSocketEndPoint = `${this.BASE_URL}\\ws`;
  client: RxStomp;
  ...
  initializeWebSocketConnection(gameId: number, player: Player) {
    ...
    config.webSocketFactory = () => {
      return new SockJs(that.webSocketEndPoint);
    };
  ...
  postAnswer(answers: Answer[]) {
    this.client.publish({
      destination: `/app/play/game/${this.gameId}/sendAnswer`,
      body: JSON.stringify({answers, player: this.player})
    });
  }
  ...
  routeQuestion(data: any) {
    const link = `/game/question/${this.gameId}/${data.questionNumber}`;
  ...
  routeResults() {
    const link = `/game/finish/${this.gameId}`;
    this.router.navigate([link]);
  }
